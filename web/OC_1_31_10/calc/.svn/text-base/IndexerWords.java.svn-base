...
public class IndexerWords {
	...
	private static String[] parseVarName(String s){
		...
		do {
			...
			if (currChar == '_'){
				for( ; currChar == '_'; currCharNum++, currChar = s.charAt(currCharNum)){
				...
				for ( ; currChar >= 'a' && currChar <= 'z'; currCharNum++, currChar = s.charAt(currCharNum)){
					indexerWords[numIndexerWords] += currChar;
					if(currCharNum >= s.length() - 1){
						hitEnd = true;
						currChar = '\0';
				...
				if (tempChar  >= 'A' && tempChar <= 'Z'){
					for( ; currChar >= 'A' && currChar <= 'Z'; currCharNum++, currChar = s.charAt(currCharNum)){
						indexerWords[numIndexerWords] += Character.toLowerCase(currChar);
						if(currCharNum >= s.length() - 1){
							hitEnd = true;
							currChar = '\0';
							break;
					...
					for ( ; Character.isLetter(currChar); currCharNum++, currChar = s.charAt(currCharNum)){
				...
				for ( ; Character.isDigit(currChar); currCharNum++, currChar = s.charAt(currCharNum)){
					indexerWords[numIndexerWords] += currChar;
					if(currCharNum >= s.length() - 1){
						hitEnd = true;
						currChar = '\0';
