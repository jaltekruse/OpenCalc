...
public class NumsAndOppsPanel extends SubPanel{
	...
	NumsAndOppsPanel(final NewCalc calcObj){
		...
		this.setBorder(BorderFactory.createTitledBorder("Math Buttons"));
				...
				if (currField != null){
					...
					if (caretPos > 0){
						if(currField.getSelectionStart() == currField.getSelectionEnd()){
							...
							String tempText = currText.substring(0, caretPos - 1);
		...
		bCon.gridx = 4;
		bCon.gridy = 4;
		currButton = new JButton("Entr");
		buttonList[getButtonListLength()] = currButton;
		super.setButtonListLength(super.getButtonListLength() + 1);
		currButton.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				if (calcObj.getFocusedComp() == 1){
					calcObj.evalCalc(calcObj.getCurrTextField().getText());
				}
				else if (calcObj.getFocusedComp() == 0){
					calcObj.changeGraphFunc();
				}
			}
		});
		*/
